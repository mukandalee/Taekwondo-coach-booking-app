package com.mukanda.taekwondocoachbooking.AthleteFragments;
...
public class FixAppointment extends AppCompatActivity {
    ...
    private String coachid,name,addr,city,spl;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setapp.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                builder.setTitle("Confirmation").setMessage("Are you sure you want set an appointment with Coach. "+name+" on "+date.getText()+" at "+time.getText()+"?");
                builder.setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        String dockey=FirebaseDatabase.getInstance().getReference().child("PendingCoachAppointments").child(coachid).push().getKey();
                        hashMap.put("CoachAppointKey",dockey);
                        String key=FirebaseDatabase.getInstance().getReference().child("PendingAthleteAppointments").child(FirebaseAuth.getInstance().getCurrentUser().getUid()).push().getKey();
                        hashMap.put("AthleteAppointKey",key);
                        FirebaseDatabase.getInstance().getReference().child("PendingAthleteAppointments").child(FirebaseAuth.getInstance().getCurrentUser().getUid()).child(key).setValue(hashMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                            @Override
                            public void onComplete(@NonNull Task<Void> task) {
                                if(task.isSuccessful()){
                                    ...
                                    hashMap.put("AthleteEmail",ReusableFunctionsAndObjects.Email);
                                    hashMap.put("AthletePhone",ReusableFunctionsAndObjects.MobileNo);
                                    hashMap.put("AthleteAppointKey",key);
                                    hashMap.put("CoachAppointKey",dockey);
                                    hashMap.put("DateAndTime",date.getText()+" "+time.getText());
                                    FirebaseDatabase.getInstance().getReference().child("PendingCoachAppointments").child(coachid).child(dockey).setValue(hashMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                                        @Override
                                        public void onComplete(@NonNull Task<Void> task) {
                                            if(task.isSuccessful()){
                                                progressDialog.dismiss();
                                                ReusableFunctionsAndObjects.showMessageAlert(FixAppointment.this, "Completed", "Appointment has been requested to Coach. "+name+" on "+date.getText()+" at "+time.getText()+". Check status in pending appointments.", "OK",(byte)1);
                                            }else{
                                                progressDialog.dismiss();
